...
| 5 | Веб-сервис AI-анализа удалённого GitHub-репо: дайджест / 🐓 rooster-роаст коммитов + Telegram + деплой на VPS (Docker/nginx); токены — в интерфейсе | 5011 |
...
- **Week 7:** ассистент разработчика — RAG по докам+коду + MCP (git, файлы,
  тикеты) + DeepSeek. Понимает проект (`/help`), AI-ревью PR (GitHub Action),
  поддержка с контекстом тикета, файловый агент, веб-сервис анализа удалённого
  GitHub-репо (дайджест / роаст) с деплоем на VPS (Docker/nginx).
