...
github_repo.py — коммиты через GitHub REST API (parse_repo из URL/owner-repo;
  GITHUB_TOKEN опц.). digest.generate(tone): 'neutral' дайджест vs 'toxic' 🔥 роаст
  (SYSTEM_TOXIC savage code review, temp 0.9; жжём по КОДУ не по авторам).
notify.py — Telegram (без токенов → skipped). app.py Flask 5011: /health/generate/publish.
Docker для VPS: Dockerfile (python-slim+gunicorn, БЕЗ git), docker-compose.yml
  (без монтирования — читаем по API; env DEEPSEEK/GITHUB/TELEGRAM; наружу 5011).
