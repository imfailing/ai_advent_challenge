...
└── week 7/             # Ассистент разработчика (RAG по докам + MCP git + DeepSeek)
    ...
    ├── day 3/           # ассистент поддержки: RAG по FAQ/докам + MCP (JSON тикетов) + контекст тикета, порт 5010
    ├── day 4/           # файловый ассистент: MCP fs (read/search/write) в песочнице, работа по цели, 2 сценария
    └── day 5/           # веб-сервис AI-анализа удалённого GitHub-репо (дайджест / 🐓 rooster-роаст) + Docker/VPS, порт 5011; токены в интерфейсе
```
...
| 3 | AI-ассистент поддержки (продукт CloudNote, вымышленный SaaS). product_docs/ — FAQ+доки (auth/plans/sync/faq). data/support.json — users+tickets (тариф, устройства, описание). support_mcp_server.py — MCP над JSON: get_ticket (тикет+пользователь), get_user, list_tickets. assistant.py SupportAssistant.ask(question, ticket_id): если ticket_id → MCP get_ticket → контекст (тариф) → RAG по докам (query=вопрос+описание тикета) → DeepSeek учитывает тариф. app.py (Flask 5010): выбор тикета + вопрос, /tickets /ask. Пример: T-1002 (Pro, про SSO) → ответ «SSO только на Business, обновите тариф»; T-1004 (Free) → «одно устройство, синхрон недоступен». test_support.py: контекст тикета подтянут, тариф учтён, источники есть. |
| 4 | Файловый ассистент — реальные операции с файлами по ЦЕЛИ. fs_mcp_server.py — MCP в песочнице workspace/ (_safe проверяет путь внутри WORKSPACE): list_files/read_file/search(regex по файлам)/write_file. agent.py FileAgent — DeepSeek tool-calling, по цели сам решает что читать/искать/писать. workspace_seed.py — детерминированная песочница (мини-проект: logger.py + 3 пользователя Logger + config.py/utils.py без докстринга). run.py: seed → снимок → агент → показ вызовов + unified diff (difflib). Сценарии: (1) найти использования Logger → docs/logger_usage.md; (2) проверить правило докстринга → compliance_report.md (находит config.py+utils.py). test_file_agent.py: агент читает ≥2 файла, создаёт отчёт, нарушители верны, seed воспроизводит. Изменения и сохраняются, и показываются как diff. |
| 5 | Веб-сервис AI-анализа УДАЛЁННОГО GitHub-репо (порт 5011). Только веб + удалённые репо (по просьбе убраны CLI run.py, локальный gitlog.py, weekly-digest.yml). ТОКЕНЫ ЗАДАЮТСЯ В ИНТЕРФЕЙСЕ, не из окружения (⚙ Токены): DeepSeek ключ (обяз.), GitHub токен (опц.), Telegram bot token + chat_id (опц.) — хранятся в localStorage, уходят на сервер только в теле запроса. github_repo.get_commits(repo, last, token) — коммиты через GitHub REST API (parse_repo из URL/owner-repo; токен опц. для приватных/лимитов; {hash,date,author,subject,body}). digest.generate(commits, title, tone, api_key): 'neutral' → дайджест (Главное/Новое/Исправления/Документация/Прочее); 'toxic' → 🔥 роаст (Вердикт/Разбор/Советы/Оценка, SYSTEM_TOXIC savage code review, temp 0.9) — В UI это «🐓 rooster режим», включён по умолчанию (переименование+дефолт, tone/промпты не менялись). Поле «Заголовок» удалено — title генерируется автоматически. notify.send_telegram(text, token, chat_id) — Telegram (без токена/chat_id → skipped). app.py (Flask): /, /health ({status:ok}), /generate {repo,last,toxic,deepseek_key,github_token}, /publish {telegram_token,telegram_chat_id}. Docker для VPS: Dockerfile (python-slim + gunicorn, БЕЗ git), docker-compose.yml (без монтирования и БЕЗ env-секретов — только PORT; наружу 5011). ГРАНИЦА безопасности: роаст по КОДУ не по авторам (без оскорблений по признакам личности); только чтение внешних репо + публикация в СВОЙ Telegram, НЕ автопостит в чужие репо. test_digest.py: ключ из env только для прогона (передаётся параметром), чтение octocat/Hello-World, оба тона, деградация Telegram, веб-эндпоинты. |
