...
└── week 7/             # Ассистент разработчика (RAG по докам + MCP git + DeepSeek)
    ...
    └── day 5/           # реальное AI-приложение: дайджест изменений репо (git log → DeepSeek → Telegram), недельный Action
```
...
| 5 | Реальное AI-приложение под свою задачу: авто-дайджест изменений репозитория. Задача — не писать release notes вручную. gitlog.py — git log (--last/--since/--range, разделители \x1f/\x1e для парсинга). digest.py — AI (DeepSeek) группирует коммиты по типам (Главное/Новое/Исправления/Документация/Прочее), выделяет главное, переписывает в user-facing язык. notify.py — публикация в Telegram Bot API (мягкая деградация: без TELEGRAM_BOT_TOKEN/CHAT_ID → status skipped, не падает). run.py → digest.md + опц. --notify. .github/workflows/weekly-digest.yml — cron понедельник 09:00 UTC + workflow_dispatch, артефакт + Telegram. test_digest.py: реальные коммиты, все разделы, skipped без токена. Работает на настоящей git-истории. Веб-интерфейс app.py (Flask, порт 5011): / UI, /health, /generate (source local/github, mode last/since/range, toxic bool), /publish. Docker: Dockerfile (git+gunicorn, safe.directory '*'), docker-compose.yml (репо монтируется read-only в /repo, REPO_DIR=/repo, наружу 5011). gitlog.REPO_ROOT = env REPO_DIR или parents[2]. РАСШИРЕНИЕ: github_repo.py — коммиты ВНЕШНЕГО GitHub-репо через REST API (parse_repo из URL/owner-repo, GITHUB_TOKEN опц. для приватных/лимитов). digest.generate(tone): 'neutral' дайджест vs 'toxic' роаст (SYSTEM_TOXIC — savage code review, temp 0.9). ГРАНИЦА безопасности: роаст жжёт по КОДУ, не по авторам (без оскорблений по признакам личности); сервис только читает внешние репо + публикует в СВОЙ Telegram, НЕ автопостит в чужие репо (харассмент). CLI: --repo, --toxic. |
