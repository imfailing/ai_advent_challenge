...
└── week 7/             # Ассистент разработчика (RAG по докам + MCP git + DeepSeek)
    ...
    └── day 5/           # веб-сервис AI-анализа удалённого GitHub-репо (дайджест / токсичный роаст) + Docker/VPS, порт 5011
```
...
| 5 | Веб-сервис AI-анализа УДАЛЁННОГО GitHub-репо (порт 5011). Только веб + удалённые репо (по просьбе убраны CLI run.py, локальный gitlog.py, weekly-digest.yml). github_repo.py — коммиты через GitHub REST API (parse_repo из URL/owner-repo; GITHUB_TOKEN опц. для приватных/лимитов; {hash,date,author,subject,body}). digest.generate(commits, title, tone): 'neutral' → дайджест (Главное/Новое/Исправления/Документация/Прочее); 'toxic' → 🔥 роаст (Вердикт/Разбор/Советы/Оценка, SYSTEM_TOXIC savage code review, temp 0.9). notify.py — Telegram (без токенов → skipped). app.py (Flask): /, /health (deepseek/github_token/telegram), /generate {repo,last,toxic,title}, /publish. Docker для VPS: Dockerfile (python-slim + gunicorn, БЕЗ git), docker-compose.yml (без монтирования — читаем по API; env DEEPSEEK_API_KEY/GITHUB_TOKEN/TELEGRAM; наружу 5011). ГРАНИЦА безопасности: роаст по КОДУ не по авторам (без оскорблений по признакам личности); только чтение внешних репо + публикация в СВОЙ Telegram, НЕ автопостит в чужие репо. test_digest.py: чтение octocat/Hello-World, оба тона, деградация Telegram, веб-эндпоинты. |
