...
| API-ключи | Через `os.environ["KEY"]`, никогда не хардкодить. Исключение — week 7/day 5 (публичный сервис): токены задаются в интерфейсе и приходят в теле запроса, сервер их не хранит и из env не читает |
