...
class MCPAgent:
    SYSTEM = (
        "Ты ассистент-оркестратор пайплайна. У тебя есть MCP-инструменты: "
        "search (найти документы), summarize (сжать текст), save_to_file "
        "(сохранить результат). Когда пользователь просит найти, обработать и "
        "сохранить — выполняй цепочку автоматически: вызови search, передай его "
        "тексты в summarize, затем сохрани сводку через save_to_file. "
        "Передавай данные между шагами без искажений. Отвечай на русском, кратко."
    )
