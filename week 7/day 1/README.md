...
| `/help <вопрос>` | отвечает на вопрос о проекте (RAG + git) |
| `<вопрос>` без `/help` | просит использовать команду — о проекте не отвечает |
...
- ✅ без `/help` — ассистент просит команду, о проекте не отвечает;
- ✅ `/help <вопрос о структуре>` — ответ **по документации** с источниками
  (`README.md`, `claude/architecture.md`, `claude/project.md`);
- ✅ `/help <вопрос про git>` — вызван **MCP** `git_branch`, ветка определена (`main`).
